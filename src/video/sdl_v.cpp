...
static void UpdatePalette(bool init = false)
{
	...
	for (int i = 0; i != _sdl_palette.count_dirty; i++) {
		pal[i].r = _sdl_palette.palette[_sdl_palette.first_dirty + i].r;
		pal[i].g = _sdl_palette.palette[_sdl_palette.first_dirty + i].g;
		pal[i].b = _sdl_palette.palette[_sdl_palette.first_dirty + i].b;
		pal[i].unused = 0;
	}

	SDL_SetColors(_sdl_surface, pal, _sdl_palette.first_dirty, _sdl_palette.count_dirty);
	...
	if (_sdl_surface != _sdl_realscreen && init) {
		 ...
		 */
		SDL_SetColors(_sdl_realscreen, pal, _sdl_palette.first_dirty, _sdl_palette.count_dirty);
	}
...
bool VideoDriver_SDL::CreateMainSurface(uint w, uint h)
{
	...
	if (_use_hwpalette == 2) {
		 ...
		 * unfortunately).
		 */
		want_hwpalette = bpp == 8 && _fullscreen && _support8bpp == Support8bpp::Hardware;
	} else {
...
void VideoDriver_SDL::ClaimMousePointer()
{
	SDL_ShowCursor(0);
}
...
void VideoDriver_SDL::InputLoop()
{
	...
	_dirkeys.Set(DirectionKey::Left, keys[SDLK_LEFT]);
	_dirkeys.Set(DirectionKey::Up, keys[SDLK_UP]);
	_dirkeys.Set(DirectionKey::Right, keys[SDLK_RIGHT]);
	_dirkeys.Set(DirectionKey::Down, keys[SDLK_DOWN]);
